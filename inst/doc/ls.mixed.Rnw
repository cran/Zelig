\SweaveOpts{results=hide, prefix.string=vigpics/lmmixed}
\include{zinput}
...
\item {\tt method:} a character string. For a linear mixed model the default is {\tt "REML"} indicating that the model should be fit by maximizing the restricted log-likelihood. The alternative is {\tt "ML"} indicating that the log-likelihood should be maximized.
...
\end{itemize}
Additionally, users may with to refer to {\tt lmer} in the package {\tt Matrix} for more information, including control parameters for the estimation algorithm and their defaults.
...
\begin{itemize}
\item The predicted values ({\tt qi\$pr}) are calculated as 
\begin{equation*}
Y_{ij} = X_{ij} \beta + Z_{ij} b_i 
\end{equation*}
...
\begin{verse}
Delia Bailey, Ferdinand Alimadhi. 2007. ``ls.mixed: Mixed effects linear regression'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
