...
To cite the \emph{oprobit.bayes} Zelig model use:
\begin{verse}
Kosuke Imai, Gary King, and Olivia Lau. 2007. ``oprobit.bayes: Bayesian Ordered Probit Regression,'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
