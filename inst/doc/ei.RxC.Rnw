...
\section{{\tt ei.RxC}: Hierarchical Multinomial-Dirichlet Ecological
  Inference Model for $R \times C$ Tables}\label{eiRxC}
...
To cite the \emph{ei.RxC} Zelig model use:
\begin{verse}
Jason Wittenberg, Ferdinand Alimadhi, and Olivia Lau. 2007. ``ei.RxC:Hierarchical Multinomial-Dirichlet Ecological Inference Model,'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
