\SweaveOpts{results=hide, prefix.string=vigpics/netnormal}
\include{zinput}
...
\section{{\tt netnormal}: Network Normal Regression for Continuous Proximity Matrix Dependent Variables}
...
\begin{verbatim}
> z.out <- zelig(y ~ x1 + x2, model = "netnormal", data = mydata) 
> x.out <- setx(z.out)
...
<<echo=TRUE, results=hide, fig=FALSE>>=
z.out <- zelig(perpower ~ friends + advice + prestige, model = "netnormal", data = friendship)
summary(z.out)
...
\begin{itemize}
\item The expected value ({\tt qi\$ev}) for the {\tt netnormal} model is the mean of simulations from the stochastic component,  
\begin{equation*}
E(Y) = \mu_{i} =  x_{i}\beta,
\end{equation*}
...
The output of each Zelig command contains useful information which you may view. For example, you run \verb{ z.out <- zelig(y ~ x, model = "netnormal", data){, then you may examine the available information in {\tt z.out} by using {\tt names(z.out)}, see the coefficients by using {\tt z.out\$coefficients}, and a default summary of information through {\tt summary(z.out)}. Other elements available through the {\tt \$} operator are listed below. 
\begin{itemize}
...
To cite the \emph{netnormal} Zelig model:
\begin{verse}
Skyler J. Cranmer. 2007. ``netnormal: Network Normal Regression for Continuous Proximity Matrix Dependent Variables,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.

\end{verse}
\input{citeZelig}
\subsection* {See also}
The network normal regression is part of the {\tt netglm} package by Skyler J. Cranmer and is built using some of the functionality of the  {\tt sna} package by Carter T. Butts \citep{ButCar01}.In addition, advanced users may wish to refer to {\tt help(netnormal)}. Sample data are fictional. 
