...
To cite the \emph{mloglm} Zelig model use:
\begin{verse}
Kosuke Imai, Gary King, and Olivia Lau. 2007. ``mloglm: Multinomial Log-Linear Regression for Contingency Table Models,'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
The multinomial logit model is part of the nnet library by Brian D.
Ripley. \citep{Ripley1996}.Advanced users may wish to refer to the R-help for
\texttt{help(multinom)} and \cite{VenRip2002}
