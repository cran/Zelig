...
To cite the \emph{blogit} Zelig model use:
\begin{verse}
Kosuke Imai,Gary King, and Olivia Lau. 2007. ``blogit: Bivariate Logistic Regression for Two Dichotomous Dependent Variable,'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
