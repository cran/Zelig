...
To cite the \emph{irtkd} Zelig model use:
\begin{verse}
...
\input{citeZelig}
\subsection*{See also}
The $k$ dimensional item-response function is part of the MCMCpack library by Andrew D. Martin and Kevin M. Quinn \citep{MarQui05}.
