...
\include{zinput}
%\VignetteIndexEntry{Generalized Additive Model for Continuous Dependent Variables}
%\VignetteDepends{Zelig, stats}
%\VignetteKeyWords{model, gam, gam.normal}
...
\section{{\tt gam.normal}: Generalized Additive Model for Continuous Dependent Variables}\label{gam.normal}

This function runs a nonparametric Generalized Additive Model (GAM) for continuous dependent variables. 

\subsubsection{Syntax}
\begin{verbatim}
> z.out <- zelig(y ~ x1 + s(x2), model = "gam.normal", data = mydata) 
> x.out <- setx(z.out)
...
\item {\tt \ldots}: additional options passed to the {\tt gam.normal} model. See the {\tt mgcv} library for details. 
\end{itemize}
...
z.out <- zelig(y~s(x0)+s(x1)+s(x2)+s(x3), model="gam.normal", data=my.data)
summary(z.out)
...
GAM models use families the same way GLM models do: they specify the distribution and link function to use in model fitting. In the case of {\tt gam.normal} a normal link function is used. Specifically, let $Y_{i}$ be the continuous dependent variable for observation $i$.  
\begin{itemize}
...
The quantities of interest for the {\tt gam.normal} model are the same as those for the standard Normal regression. 
\begin{itemize}
\item The expected value ({\tt qi\$ev}) for the {\tt gam.normal} model is the mean of simulations from the stochastic component,  
\begin{equation*}
...
\item The first difference ({\tt qi\$fd}) for the GAM Normal model is defined as 
\begin{equation*}
...
The output of each Zelig command contains useful information which you may view. For example, if you run {\tt z.out <- zelig(y \~{} x, model = "gam.normal", data)}, then you may examine the available information in {\tt z.out} by using {\tt names(z.out)}, see the coefficients by using {\tt coefficients(z.out)}, and a default summary of information through {\tt summary(z.out)}. Other elements available through the {\tt \$} operator are listed below. 
\begin{itemize}
...
To cite the \emph{gam.normal} Zelig model:
\begin{verse}
Skyler J. Cranmer. 2007. ``gam.normal: Generalized Additive Model for Dichotomous Dependent Variables,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
