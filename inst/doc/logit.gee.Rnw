\SweaveOpts{results=hide, prefix.string=vigpics/logit_gee}
\include{zinput}
%\VignetteIndexEntry{Generalized Estimating Equation for Logistic Regression}
%\VignetteDepends{Zelig, gee}
%\VignetteKeyWords{model,generalized estimating equation, logistic regression}
...
@
<<FirstDifference.summary.sim>>=
summary(s.out2)
@
\begin{center}
<<label=FirstDifferencePlot,fig=true,echo=true>>=
plot(s.out2)
@
\end{center}
...
\begin{verse}
Patrick Lam. 2007. ``logit.gee: Generalized Estimating Equation for Logit Regression,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
