...
%\VignetteKeyWords{model, gam, gam.probit}
...
\section{{\tt gam.probit}: Generalized Additive Model for Dichotomous Dependent Variables}

This function runs a nonparametric Generalized Additive Model (GAM) for dichotomous dependent variables. 

\subsubsection{Syntax}
\begin{verbatim}
> z.out <- zelig(y ~ x1 + s(x2), model = "gam.probit", data = mydata) 
> x.out <- setx(z.out)
...
\item {\tt \ldots}: additional options passed to the {\tt gam.probit} model. See the {\tt mgcv} library for details. 
\end{itemize}
...
GAM models use families the same way GLM models do: they specify the distribution and link function to use in model fitting.  In the case of {\tt gam.probit} a normal link function is used. Specifically, let $Y_{i}$ be the binary dependent variable for observation $i$ which takes the value of either 0 or 1.
\begin{itemize}
...
The quantities of interest for the {\tt gam.probit} model are the same as those for the standard normal regression. 
\begin{itemize}
\item The expected value ({\tt qi\$ev}) for the {\tt gam.probit} model is the mean of simulations from the stochastic component,  
\begin{equation*}
\pi_{i} = \pmb{\Phi} \left( x_{i}\beta + \sum_{j=1}^{J} f_j(Z_j) \right).
\end{equation*}
...
\item The first difference ({\tt qi\$fd}) for the GAM probit model is defined as 
\begin{equation*}
...
The output of each Zelig command contains useful information which you may view. For example, if you run {\tt z.out <- zelig(y \~{} x, model = "gam.probit", data)}, then you may examine the available information in {\tt z.out} by using {\tt names(z.out)}, see the coefficients by using {\tt coefficients(z.out)}, and a default summary of information through {\tt summary(z.out)}. Other elements available through the {\tt \$} operator are listed below. 
\begin{itemize}
...
To cite the \emph{gam.probit} Zelig model:
\begin{verse}
Skyler J. Cranmer. 2007. ``gam.probit: Generalized Additive Model for Dichotomous Dependent Variables,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
