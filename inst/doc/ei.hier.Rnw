...
To cite the \emph{ei.hier} Zelig model use:
\begin{verse}
Ben Goodrich and Ying Lu. 2007. ``ei.hier: Hierarchical Ecological Inference Model for 2x2 tables'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
