...
To cite the \emph{ei.dynamic} Zelig model use:
\begin{verse}
Ben Goodrich and Ying Lu. 2007. ``ei.dynamic: Quinn's Dynamic Ecological Inference,'' in Kosuke Imai,Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
\end{verse}
\input{citeZelig}
\subsection*{See also}
