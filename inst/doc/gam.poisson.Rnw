...
%\VignetteKeyWords{model, gam, gam.poisson}
...
\section{{\tt gam.poisson}: Generalized Additive Model for Count Dependent Variables}\label{gam.poisson}

This function runs a nonparametric Generalized Additive Model (GAM) for count dependent variables. 

\subsubsection{Syntax}
\begin{verbatim}
> z.out <- zelig(y ~ x1 + s(x2), model = "gam.poisson", data = mydata) 
> x.out <- setx(z.out)
...
\item {\tt \ldots}: additional options passed to the {\tt gam.poisson} model. See the {\tt mgcv} library for details. 
\end{itemize}
...
z.out <- zelig(y~s(x0)+s(x1)+s(x2)+s(x3), H=diag(0.5,37), 
   model="gam.poisson", data=my.data) 
summary(z.out)
plot(z.out,pages=1,residuals=TRUE)
...
The quantities of interest for the {\tt gam.poisson} model are the same as those for the standard Poisson regression. 
\begin{itemize}
\item The expected value ({\tt qi\$ev}) for the {\tt gam.poisson} model is the mean of simulations from the stochastic component,  
\begin{equation*}
...
\item The first difference ({\tt qi\$fd}) for the GAM Poisson model is defined as 
\begin{equation*}
...
The output of each Zelig command contains useful information which you may view. For example, if you run {\tt z.out <- zelig(y \~{} x, model = "gam.poisson", data)}, then you may examine the available information in {\tt z.out} by using {\tt names(z.out)}, see the coefficients by using {\tt coefficients(z.out)}, and a default summary of information through {\tt summary(z.out)}. Other elements available through the {\tt \$} operator are listed below. 
\begin{itemize}
...
To cite the \emph{gam.poisson} Zelig model:
\begin{verse}
Skyler J. Cranmer. 2007. ``gam.poisson: Generalized Additive Model for Dichotomous Dependent Variables,'' in Kosuke Imai, Gary King, and Olivia Lau, ``Zelig: Everyone's Statistical Software,'' \url{http://gking.harvard.edu/zelig}.
