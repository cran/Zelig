\SweaveOpts{results=hide, prefix.string=vigpics/threesls}
\include{zinput}
%\VignetteIndexEntry{ Three Stage Least Squares}
%\VignetteDepends{Zelig}
%\VignetteKeyWords{model,continuous, least squares,error term}
...
\begin{center}
<<label=Examplesthreesls, fig=true, echo=false>>=
plot(s.out)
@ 
\end{center}
\clearpage
...
\subsection* {See also}
The threesls function is adapted from the \texttt{systemfit} library by Jeff Hamann
