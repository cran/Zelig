...
user.prompt()
z.out <- zelig(y~s(x0)+s(x1)+s(x2)+s(x3), model="gam.logit", data=my.data)
user.prompt()
summary(z.out)
user.prompt()
plot(z.out, pages=1)
...
user.prompt()
z1.out <- zelig(y~s(x0)+s(x1)+s(x2), model="gam.logit", data=my.data)
z2.out <- zelig(y~s(x1)+s(x2), model="gam.logit", data=my.data)
